...
int shortUnhappySounds[]={S_DISGRUNTLED,S_CONFUSED_SHORT};
...
int getRandomSound(int soundSetName[]) {
  return soundSetName[random(sizeof(soundSetName) / sizeof(soundSetName[0]))];
}
 
void CuteBuzzerSoundsClass::playRandom(int soundSetName) {
  switch(soundSetName) {
    case SG_JOYFUL:
      cute.play(getRandomSound(joyfulSounds));
      break;
    case SG_UNHAPPY:
      cute.play(getRandomSound(shortUnhappySounds));
      break;
  }
}
